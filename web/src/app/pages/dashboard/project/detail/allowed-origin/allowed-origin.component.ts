...
@Component({
  selector: 'app-allowed-origin',
  templateUrl: './allowed-origin.component.html',
  styleUrls: ['./allowed-origin.component.less']
})
export class AllowedOriginComponent implements OnInit {
  ...
  addAllowedOrigin(_id: string | any, data: { origin: string }) {
    ...
    // @ts-ignore
    this.projectService.create(data, headers, `/${_id}/add-allowed-origin`).subscribe((
      projectUpdatedResponseInterface: any
    ) => {
      this.projectResponseInterface = projectUpdatedResponseInterface;
      this.allowedOriginAlert = {
        type: 'success', message: `Successfully new origin added.`
      };
      this.formGroup.reset();
      this.nzLoadingAdd = false;
  ...
  removeAllowedOrigin(_id: string | any, data: { origin: string }) {
    ...
    const headers = new HttpHeaders()
      ...
      .append('Access-Control-Allow-Headers', 'Content-Type');
    // @ts-ignore
    this.projectService.delete(`/${_id}`, headers, `/remove-allowed-origin`, data).subscribe((
      projectUpdatedResponseInterface: any
    ) => {
      this.projectResponseInterface = projectUpdatedResponseInterface;
      this.allowedOriginAlert = {
        type: 'success', message: `Successfully origin removed.`
      };
      this.nzLoadingRemove = false;
    }, (httpErrorResponse: HttpErrorResponse) => {
      this.nzLoadingRemove = false;
