...
@flask.route("/<key>", methods=["POST"], subdomain="mainnet")
def mainnet_http(key: str) -> Response:
    ...
    if is_invalid:
        add_statistics(
            project=project, network="mainnet", request_method="invalid", is_valid=False
        )
        return Response(
    ...
    custom_http_provider: CustomHTTPProvider = CustomHTTPProvider(
        endpoint_uri=URI(f"http://{flask.config['HOST_JANUS_MAINNET']}:{flask.config['PORT_JANUS_MAINNET']}"),
        request_kwargs={
            "timeout": flask.config['TIMEOUT_JANUS_MAINNET']
        }
    )
    ...
    add_statistics(
        project=project, network="mainnet",
        request_method=(
...
@sock.route("/<key>", subdomain="mainnet")
def mainnet_websocket(websocket: simple_websocket.ws.Server, key: str) -> None:
    project: dict = database["project"].find_one(
        filter=dict(key=key)
    )
    if project is None:
    ...
    custom_websocket_provider: CustomWebsocketProvider = CustomWebsocketProvider(
        endpoint_uri=URI(f"ws://{flask.config['HOST_JANUS_MAINNET']}:{flask.config['PORT_JANUS_MAINNET']}")
    )
